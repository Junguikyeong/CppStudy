...
int main()
{
    ...
    // 001 | 100 = 00101 | 10000 = 10101  그러므로 버프를 추가할때는 OR연산을 해야한다.
    int iBuf = iAttack | iHP | iCritical;
    //연산자 축약형 : 연산자를 줄여서 사용할 수 있다.
   
    /*
    XOR을 이용해서 스위치 기능처럼 쓸 수가 있다.
    ...
    /*
    컴퓨터는 2나눗셈보다 0.5곱셈하는게 더 빠르다.
    쉬프트 연산자 : <<, >> 값대 값을 연산하여 값으로 나오게 된다.
    이 연산자 또한 이진수 단위의 연산을 하게된다.
    20 = 10100
        2의n승의 곱셈
    20 << 2 = 1010000 = 80
    20 << 3 = 10100000= 160 
    ...
    int iNumber = iHigh;
    //iNumber에는 187이 들어가있다. 이 값을 < 방향으로 이동시키면
    //상위 16비트에 값이 들어가게 된다.
    ...
    if문 : 조건을 체크해주는 기능이다.
    형태 : if(조건식){}(코드블럭)
    if문은 조건식이 true가 될 경우 코드블럭안의 코드가 동작된다.
    ...
    /*
    else : if문과 반드시 같이 사용이 되어야한다.
    if문 조건이 false일경우 else가 있다면 else구문안의 코드가 동작된다.
    else if : if문과 반드시 같이 사용이 되어야 한다.
    if문 아래, else보다는 위에 있어야 한다.
    else if는 자신의 위에 있는 조건식이 false일 경우 다음 else if문을 체크한다.
    else if는 몇개든 사용이 가능하다.*/

    if(false)
        cout << "if문 조건의 true입니다." << endl;
    else
        cout << "if문의 조건이 false입니다." << endl;
    ...
    //강화확률 :  업그레이드가 0 ~ 3: 100%, 4~6 : 40%, 7~9 : 10%
    //10 ~ 13 : 1%, 14 ~ 15 : 0.01%
